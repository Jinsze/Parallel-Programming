...
static inline void resetTimer(hwtimer_t* timer)
{
	// hrtime_t start = 0;
	// hrtime_t end = 0;
	timer->start = 0;
...
static inline void startTimer(hwtimer_t* timer)
{
	timer->start = _rdtsc();
}

static inline void stopTimer(hwtimer_t* timer)
{
	timer->end = _rdtsc();
}

static inline uint64_t getTimerTicks(hwtimer_t* timer)
{
	return timer->end - timer->start;
}

static inline uint64_t getTimerNs(hwtimer_t* timer)
{
	if (timer->cpuMHz == 0) {
